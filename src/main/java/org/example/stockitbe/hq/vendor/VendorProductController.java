...
public class VendorProductController {
    ...
    public BaseResponse<List<VendorProductDto.ListRes>> list(
            @Parameter(description = "공급처 코드 필터 — 지정 시 해당 공급처 매핑만", example = "VND-001") @RequestParam(required = false) String vendorCode,
            @Parameter(description = "매핑 상태 필터 (ACTIVE/INACTIVE)", example = "ACTIVE") @RequestParam(required = false) VendorProductStatus status) {
    ...
    public BaseResponse<VendorProductDto.DetailRes> detail(
            @Parameter(description = "공급처 제품 코드", example = "VP-TOP-SS-001-V00") @PathVariable String code) {
